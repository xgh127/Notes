...
user_defined_random_linear = Environment(
    ...
    r_max=2 * m,  # 奖励的上限
    reward_fn=lambda env, t, L_t: r1 @ L_t + r2,  # 奖励函数,采用的事
    transition_fn=lambda env, t, L_t: torch.nn.Softmax(dim=-1)(p1 @ L_t + p2)  # 状态转移函数
)

# 使用MF-OMO算法求解环境
solns, expls, runtimes = MFOMO().solve(user_defined_random_linear, max_iter=300, verbose=True)
...
plt.title("User-Defined Environment - MFOMO Algorithm")
plt.show()