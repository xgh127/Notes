...
# 创建环境实例
env = AutonomousDrivingEnvironment(T=4, S=(n,), A=(n,), mu0=torch.ones(n) / n, r_max=2 * m,  reward_fn=lambda env, t, L_t: r1 @ L_t + r2,  # 奖励函数
    transition_fn=lambda env, t, L_t: torch.nn.Softmax(dim=-1)(p1 @ L_t + p2)  # 状态转移函数)

# 使用MF-OMO算法求解环境
solutions, exploitabilities, runtimes =  MFOMO().solve.solve(env, max_iter=300, verbose=True)
